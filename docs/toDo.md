...
- QuestRepository 분리 -> 전체 퀘스트, 플레이어 퀘스트 (퀘스트 히스토리 따로 저장할지 고민)
  - inProgressQuests 외에는 상태 추적 불가능
    - 완료된 퀘스트가 사라져버림 → 플레이어 히스토리 관리 불가
    - 같은 Quest가 다시 뜨는 게임이라면 과거 기록을 잃어서 밸런스 이슈 가능
    - 진행중 리스트에서만 교체 하지말고 Repository가 상태별 교체 및 히스토리 관리가능하게 만들기
      - 필터만으로 여러 상태 조회 가능
      - DB 붙여도 자연스럽게 마이그레이션 가능
      - JSON 기반 로딩이라 큰 문제 없지만, 장기적으로는 DB 설계나 가비지 컬렉션 고려
      - 단일 리스트에서 상태 기반 필터링 비용이 커진다 -> map구조로?
